...
public class ManagementController : ControllerBase
{
    ...
    private static string BuildDirectChatPrompt(AIHost.Config.ModelInstance model, ChatRequest request)
    {
        ...
        if (isQwen)
        {
            ...
            // Official Qwen Jinja template (from model files):
...
#pragma warning restore CA1416
    ...
    /// <summary>
    /// Get VRAM / device memory information for all loaded models.
    /// </summary>
    [HttpGet("system/vram")]
    public IActionResult GetVramInfo()
    {
        try
        {
            ...
            foreach (var (name, instance) in models)
            {
                if (instance.Device == null) continue;

                var memInfo = instance.Device.GetMemoryInfo();
                devices.Add(new
                {
                    model = name,
                    device_name = instance.Device.ProviderName,
