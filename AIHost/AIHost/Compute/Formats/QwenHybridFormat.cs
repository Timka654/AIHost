...
public class QwenHybridFormat : ITransformerFormat
{
    ...
    /// SIMD matvec with TRANSPOSED weights: result[j] += x[off+k] * wt[j*dm+k]
    ...
    static Tensor TypeB(TransformerBase t, Tensor x, int g, int li, uint pos, KVCache? kvc)
    {
        ...
        var oW = t.TempF32Named($"blk.{g}.attn_output.weight"); var pnW = t.TempF32Named($"blk.{g}.post_attention_norm.weight");
        ...
        var x1n = o.Clone(x1, "pnB"); o.LayerNorm(x1n, pnW.tensor); if (!pnW.isScratch) o.DeferExternal(pnW.tensor);
    ...
    static Tensor SSM(TransformerBase t, Tensor xr, Tensor xn, int g, SSMState ss, int sl)
    {
        ...
        var xd=xn.ReadData();var Wdt=Ld($"blk.{g}.ssm_alpha.weight");var Wb=Ld($"blk.{g}.ssm_beta.weight");var ssA=Ld($"blk.{g}.ssm_a");var dtB=Ld($"blk.{g}.ssm_dt.bias");var sNw=Ld($"blk.{g}.ssm_norm.weight");var cW=Ld($"blk.{g}.ssm_conv1d.weight");var wZ=Ld($"blk.{g}.attn_gate.weight");var wQ=Ld($"blk.{g}.attn_qkv.weight");var wO=Ld($"blk.{g}.ssm_out.weight");

        // Transpose for SIMD matvecs (freed after Phase 1)
        var wZT=Tr(wZ,dm,VD);var WbT=Tr(Wb,dm,NVH);var WdtT=Tr(Wdt,dm,NVH);var wQT=Tr(wQ,dm,CD);var wOT=Tr(wO,VD,dm);

        // Phase 1: parallel matvecs
        var zA=new float[sl*VD];var bA=new float[sl*NVH];var aA=new float[sl*NVH];var gA=new float[sl*NVH];var qA=new float[sl*CD];
        Parallel.For(0,sl,ti=>{int oo=ti*dm;MV(zA.AsSpan(ti*VD,VD),xd,oo,wZT,VD,dm);MV(bA.AsSpan(ti*NVH,NVH),xd,oo,WbT,NVH,dm);MV(aA.AsSpan(ti*NVH,NVH),xd,oo,WdtT,NVH,dm);
            for(int n=0;n<NVH;n++){bA[ti*NVH+n]=1f/(1f+MathF.Exp(-bA[ti*NVH+n]));aA[ti*NVH+n]=MathF.Log(1f+MathF.Exp(aA[ti*NVH+n]+dtB[n]));gA[ti*NVH+n]=aA[ti*NVH+n]*ssA[n];}
            MV(qA.AsSpan(ti*CD,CD),xd,oo,wQT,CD,dm);});
        ...
        for(int ti=0;ti<sl;ti++){int bt=ti*NVH,at=ti*NVH,gt=ti*NVH,zt=ti*VD,qt=ti*CD;
        ...
        var rd=new float[sl*dm];Parallel.For(0,sl,ti=>{MV(rd.AsSpan(ti*dm,dm),ys,ti*VD,wOT,dm,VD);});
